...
void TensorRTModule::doInference(float *input, float *output) {
    CHECK(cudaMemcpyAsync(deviceInput, input,
        m_params.batchSize * m_params.channels * m_params.input_h * m_params.input_w * sizeof(float),
...
void TensorRTModule::initialize(const TRTParams &buildConfig) {
    ...
    std::string engineFilePath =
        m_params.onnxFilePath.substr(0, m_params.onnxFilePath.find_last_of('.')) + ".engine";
    ...
    m_logger->log(Severity::kINFO, "TensorRTModule initialized. Resources allocated.");
}

void TensorRTModule::infer() {
    std::vector<float> input(m_params.batchSize * m_params.channels * m_params.input_h * m_params.input_w, 1.0f);
    ...
    for (int i = 1; i < m_params.output_size; ++i)
        if (output[i] > maxProb) { maxProb = output[i]; maxClass = i; }
